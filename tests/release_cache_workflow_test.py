...
CI_CACHE_WORKFLOW = ROOT / ".github/workflows/publish-ci-cache.yml"
...
class ReleaseCacheWorkflowTest(unittest.TestCase):
    ...
    def test_ci_cache_is_published_only_after_successful_main_ci(self) -> None:
        workflow = CI_CACHE_WORKFLOW.read_text()

        self.assertIn("workflow_run:", workflow)
        self.assertIn("workflows: [CI]", workflow)
        self.assertIn("github.event.workflow_run.conclusion == 'success'", workflow)
        self.assertIn("github.event.workflow_run.head_branch == 'main'", workflow)
        self.assertIn("nix-tools-cache", workflow)
        self.assertIn("environment: cache-publishing", workflow)
        self.assertIn("NIX_CACHE_PRIVATE_KEY", workflow)
        self.assertIn(".github/workflows/scripts/build-ci-cache.sh", workflow)
