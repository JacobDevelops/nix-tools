...
impl Source {
    const fn variant(&self) -> SourceVariant {
        match self {
            ...
            Self::FetchUrl { .. } => SourceVariant::FetchUrl,
            Self::FetchGit { .. } => SourceVariant::FetchGit,
            Self::FetchGitHub { .. } => SourceVariant::FetchGitHub,
            Self::FetchTarball { .. } => SourceVariant::FetchTarball,
            Self::CopyToStore { .. } => SourceVariant::CopyToStore,
        }
    }
}
...
fn render(
    ...
    let variants = remote_packages
        .iter()
        .map(|(_, package)| package.source.variant())
        .collect::<BTreeSet<_>>();
    let mut output = render_prelude(&variants);
...
fn render_prelude(variants: &BTreeSet<SourceVariant>) -> String {
    ...
    if variants.contains(&SourceVariant::CopyToStore) {
        output.push_str("  copyPathToStore,\n");
    }
    if variants.contains(&SourceVariant::FetchGitHub) {
        output.push_str("  fetchFromGitHub,\n");
    }
    if variants.contains(&SourceVariant::FetchGit) {
        output.push_str("  fetchgit,\n");
    }
    if variants.contains(&SourceVariant::Npm) || variants.contains(&SourceVariant::FetchUrl) {
    ...
    if variants.contains(&SourceVariant::Npm) {
        output.push_str(
    ...
    if variants.contains(&SourceVariant::FetchUrl) {
        output.push_str(
    ...
    if variants.contains(&SourceVariant::FetchTarball) {
        output.push_str(
